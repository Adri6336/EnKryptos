...
#from random import randint # Needed for hacking defense (obsolete, but kept as a note just in case)
...
class NewprojectApp:
    ...
    def encrypt(self): # This function will encrypt files
        ...
        except: # If something goes wrong, alert user and exit the function.
            self.changeText(text='ERROR: PROCESS ABORTED\nREASON: UNKNOWN')
    ...
    def decrypt(self): # This function will decrypt files
        try:
            ...
            self.getPathExt()
            if self.tempExten != '.aes':
                self.changeText(text="ERROR: PROCESS ABORTED\nREASON: ATTEMPT TO DECRYPT A FILE THAT'S EITHER UNENCRYPTED OR ENCRYPTED WITH ANOTHER PROGRAM")
                return
            ...
            # 5. Display data if it comes from a text file
            if self.tempExten == '.txt':
                self.viewDecrypt(path=finPath)
    ...
    def encryptMessage(self): # This function will encrypt a new message
        ...
        try:
            ...
            messageMod.encryptFile(ramFile, 'encrypted_files/' + subject + '.txt.aes', self.password, bit)
        except:
            self.changeText(text='ERROR: PROCESS ABORTED\nREASON: UNKNOWN')
    ...
    I'm keeping this function here as a note, just in case I find it useful in the future.
    
    def getRandName(self, sizeName, sizeExten): # Creates a randomized name of a specific size

        # 2.1 create random file name of length 4-16
        randFileName = self.randGen(size=sizeName)
        
        # 2.2 create random file extension of length 3-7
        randExten = '.' + self.randGen(size=sizeExten)

        # 2.3 names into a single file name variable

        fullName = randFileName + randExten

        return fullName

    def randGen(self, size):
    ...
    def changeText(self, text, ee=False): # Gets text and prints it to textbox (ee = easter egg)
        ...
        if ee == True: # If this is an easter egg (the only one is a list), convert the list into a string and clean it 
            textStr = ''
            for section in text: 
                textStr += section # Concatenate every element of list to empty string
    ...
    def viewDecrypt(self, path): # Display a .txt file's data in the view
        ...
        self.decryView.config(state='normal')
        self.decryView.delete('1.0', 'end')
        self.decryView.insert('0.0', content)
        self.decryView.config(state='disabled')
...
if __name__ == '__main__':
    ...
    root.wm_title("EnKryptos v0.3") # Sets the title of the window to the string included as an argument

...
class NewprojectApp:
    ...
    def encryptMessage(self): # This function will encrypt a new message
        ...
        oldFile = open('tsgycg.ftf', 'w')
        oldFile.write(toEncode)
        oldFile.close()

        #3. Encrypt message and save to aes file
        bit = 64 * 1024
        pyAesCrypt.encryptFile('tsgycg.ftf', subject + '.txt.aes', self.password, bit)
    
        #4. Corrupt original file's data
        oldFile = open('tsgycg.ftf', 'w')
        corrupt = open('random.txt', 'r')
        ...
        if os.path.exists('tsgycg.ftf') == True:
            os.remove('tsgycg.ftf')
            #success = open('success.txt', 'w')
            #success.write('Fuck that old file. It\'s gone now!')
            #success.close()
...
if __name__ == '__main__':
    ...
    root.wm_title("EnKryptos v0.2") # Sets the title of the window to the string included as an argument

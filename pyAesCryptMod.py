...
# I'm not proud of the quality of these modifications, but hey it got the job done
# Changelog:
#   - Deleted some stuff
...
# pyAesCrypt module
#print('Modified Version of pyAesCrypt has been Loaded')
...
def encryptFile(infile, outfile, passw, bufferSize):
    ...
    #print('Attempting to use "with" with getvalue()')
    with infile as fIn:
        #print('Attempting to see if file good')
        # check that output file does not exist
        # or that, if exists, is not the same as the input file
        # (i.e.: overwrite if it seems safe)
        
        try:
            #print('Trying to make output file.')
            with open(outfile, "wb") as fOut:
                # encrypt file stream
                #print('Calling Encrypting Function Now')
...
def encryptStream(fIn, fOut, passw, bufferSize):
    ...
    while True:
        ...
        #print('Now Trying to Encrypt')
        strin = fIn.getvalue()
        fInT = BytesIO(bytes(strin, 'utf-8'))
        fIn = fInT

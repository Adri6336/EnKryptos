# Note: the Linux version has slightly different code for the GUI. Everything
# else is the same.
...
class NewprojectApp:
    ...
    tempExten = ''
    password = ''
    keyPass = '[REDACTED]'  # Replace this with a new random password
    invalid = ['!', '@', '#', '$', '%',
                '^', '&', '*', '(', ')',
                '+', '=', '\\', '/', ':'
                ';', '"', "'", '<', '>',
    ...
    def __init__(self, master=None):
        ...
        # If we've got a key already
        if os.path.isfile('key/key.EDK'):
            # Decrypt key and assign contents to password. Make bytestream work with decryption
            bit = 64 * 1024
            key = fakeFile()
        ...
        # If we don't have a key
            ...
            # 3. Encrypt message and save to .EDK file in encryption folder
            bit = 64 * 1024
            try:
        ...
        # Welcome Message
        if firstBoot:
            self.changeText(text=('Welcome to EnKryptos! Please be sure to check out the readme.txt file ' +
                                  'for more information on how to use this application. Happy encrypting!'))
    ...
    def decrypt(self): # This function will decrypt files
        try:
            ...
            self.tempExten = ''
            pathway = filedialog.askopenfilename(initialdir='', title='Select a File to Decrypt', filetypes=(("Encrypted Files", "*.aes"), ('Invalid Files', '*.*')))
            ...
            if not isMessage:
                ...
                if pathway != '' and pathway != 'No File Selected':
                    ...
                    self.changeText(text='File Successfully Decrypted!')
...
if __name__ == '__main__':
    ...
    root.wm_title("EnKryptos v0.3.1") # Sets the title of the window to the string included as an argument
